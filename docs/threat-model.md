...
| `api_key` | bearer capability to call the API as an org | Postgres `api_keys`, stores **only `sha256` hash** (`key_hash`) | RLS + FORCE; random ≥256-bit CSPRNG, hashed at rest, constant-time compare, shown once; write path is `webhook_app`, verify path is the column-scoped, SELECT-only `webhook_authn` (ADR-0008 Option B) | until revoked/expired |
...
- **A leaked `webhook_authn` credential enumerates api-key metadata across tenants** — the bearer-verify role holds a `FOR SELECT USING(true)` policy on `api_keys` (verification is org-discovery-by-hash, so it can't be tenant-scoped before the lookup), and a column-level grant on `(key_hash, org_id, scopes, expires_at, revoked_at)` only. So a leaked authn credential can read those columns for every org, but it **cannot forge or use a key** (`key_hash` is a non-reversible sha256 of a ≥256-bit CSPRNG secret) and **cannot write** (no INSERT/UPDATE/DELETE grant), and it never sees the display metadata (`name`/`prefix`/`start`/timestamps). Accepted and documented per ADR-0008 (Option B).
