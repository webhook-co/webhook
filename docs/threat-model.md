...
- **Dedup collapse runs on unverified input (event-suppression primitive).** Because the dedup key is derived before signature verification, an actor who can POST to an endpoint (URL is a bearer secret that can leak) may craft requests that share a key — an id-header value, or a configured `fields` value — to force distinct forged events to collapse, and could suppress a genuine event that happens to share the key (`INSERT ... DO NOTHING`). Bounded by rate-limit + soft-cap; `dedup_strategy` is recorded so a collapsed event is explainable in inspection. Verified-gated collapse is a future hardening (ADR-0104); dedup remains best-effort load-reduction, never an exactly-once/authenticity guarantee.
- **Forged-payload overwrite via the dedup-key-named R2 object.** The payload object is named `hash(endpoint_id, dedup_key)` and the R2 PUT precedes the ON CONFLICT gate, so a forged same-dedup-key request with a different body can overwrite a legitimate captured payload (the dedup key is attacker-influenceable via id headers). Pre-existing; bounded by rate-limit + the requirement to know/guess a victim's dedup key. The hardening (key the object on `dedup_key ∥ content_hash` AND switch readers to the stored `payloadR2Key`) is planned as its own slice (ADR-0104), sequenced separately because the reader change spans the delivery dispatcher + replay.
