...
- **Forged-payload overwrite via the dedup key.** The dedup key is derived from **unverified**, attacker-influenceable input and the R2 PUT precedes the ON CONFLICT gate, so naming the payload object by the dedup key would let a forged key-collision overwrite a legitimate payload. Defeated by **content-addressing** the R2 object (`sha256(body)`, not the dedup key, ADR-0104): two distinct payloads never share an object; identical retries still coalesce. Per-event `payloadR2Key` is stored so reads are unaffected.
