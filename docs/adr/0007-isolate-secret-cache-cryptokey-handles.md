...
- status: accepted
- date: 2026-06-12
- scope: `packages/shared` (envelope/KMS seam), KMS work
- note: refines the compliance-by-architecture ADR's isolate-caching decision
...
The cache holds **non-extractable `CryptoKey` handles, never raw key bytes**. The
...
format. BAA tenants default to a tighter or zero cache (unwrap-per-use), keyed off
...
- The concrete KMS custodian (AWS KMS) and the LRU implementation come later;
  this decision fixes the seam, the envelope format, and the handle rule.
