...
import {
  ...
  REQUIRED_POSITIVE,
  REQUIRED_NEGATIVE,
} from "./plugin-test-cases-guard.mjs";
...
test("the real submission cases pass against the real engine and the real skill", async () => {
  assert.deepEqual(await check(), []);
});

test("every real fixture is reachable and actually runs", async () => {
  // Floor: if a fixture id stopped resolving, the per-case replay below would be checking nothing.
  assert.ok(Object.keys(FIXTURES).length >= REQUIRED_POSITIVE);
  for (const [id, fixture] of Object.entries(FIXTURES)) {
    const result = await runFixture(fixture);
    assert.equal(result.error, undefined, `fixture ${id} has no adapter`);
    assert.equal(typeof result.ok, "boolean", `fixture ${id} produced no verdict`);
  }
});

test("unreadable cases fail instead of passing vacuously", async () => {
  const problems = await check(sources({ casesSource: null }));
  assert.equal(problems.length, 1);
  assert.match(problems[0], /cannot read/i);
});

test("an unreadable skill fails, because every refusal check would otherwise pass over nothing", async () => {
  const problems = await check(sources({ skillSource: null }));
  assert.equal(problems.length, 1);
  assert.match(problems[0], /cannot read/i);
});

test("the exact 5/3 counts are enforced in both directions", async () => {
  ...
  for (const [cases, word] of [
    ...
    const problems = await check(sources({ casesSource: JSON.stringify(cases) }));
    assert.equal(problems.length, 1);
    assert.ok(problems[0].includes(word), `should name ${word}: ${problems[0]}`);
  }
});

test("a fixtureId that resolves to nothing fails rather than being skipped", async () => {
  const cases = {
    ...OK_CASES,
    positive: [
...
test("a refusal the skill does not state fails", async () => {
  const problems = await check(sources({ skillSource: "this skill states nothing of the sort" }));
  assert.equal(problems.length, REQUIRED_NEGATIVE);
  assert.match(problems[0], /requires the skill to state/);
});

test("a case missing a required field fails", async () => {
