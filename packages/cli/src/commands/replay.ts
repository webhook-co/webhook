...
export const replayCommand = buildCommand<ReplayFlags, [string], AppContext>({
  async func(this: AppContext, flags, eventId) {
    ...
    // The selector's resolved org isn't needed locally — it has already done its job by picking the profile
    // whose credential binds the org, and replay builds no dashboard link.
    const { profile } = await resolveRequestProfile(this, flags);
