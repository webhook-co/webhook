...
describe("withHeartbeat", () => {
  it("runs the job and reports success", async () => {
    const f = okFetch();
    let ran = 0;
    await withHeartbeat(env, "anchor", async () => void ran++, f as unknown as typeof fetch);
    expect(ran).toBe(1);
  ...
  it("still runs the job when reporting is switched off", async () => {
    const f = okFetch();
    let ran = 0;
    await withHeartbeat({}, "anchor", async () => void ran++, f as unknown as typeof fetch);
    expect(ran).toBe(1);
    expect(f).not.toHaveBeenCalled();
  });
  ...
  it("bounds the report with a timeout signal", async () => {
    const f = okFetch();
    await withHeartbeat(env, "anchor", async () => {}, f as unknown as typeof fetch);
