...
describe("makeStripeClient — the mode/key guard is UNSKIPPABLE (it lives in the constructor)", () => {
  ...
  it("throws on a LIVE key in test mode (it would charge real cards)", () => {
    expect(() => makeStripeClient({ mode: "test", secretKey: "sk_live_x" })).toThrow(
      /does not match BILLING_MODE/,
    );
  });

  it("throws on a TEST key in live mode (it would take no money)", () => {
    expect(() => makeStripeClient({ mode: "live", secretKey: "sk_test_x" })).toThrow(
      /does not match BILLING_MODE/,
    );
  });

  it("throws when billing is off, and on a publishable key used as a secret", () => {
    expect(() => makeStripeClient({ mode: "off", secretKey: "sk_test_x" })).toThrow();
    expect(() => makeStripeClient({ mode: "live", secretKey: "pk_live_x" })).toThrow();
  });

  it("never names the key or its prefix in the error", () => {
    try {
      makeStripeClient({ mode: "test", secretKey: "sk_live_SUPERSECRET" });
  ...
  it("constructs fine for a matching pair", () => {
    expect(() => makeStripeClient({ mode: "live", secretKey: "sk_live_x" })).not.toThrow();
    expect(() => makeStripeClient({ mode: "test", secretKey: "sk_test_x" })).not.toThrow();
  });
});
