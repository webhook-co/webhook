...
describe("migration 0077 reversibility (effect, not just structure)", () => {
  it(
    "down restores case-SENSITIVE uniqueness; up re-applies the case-insensitive guarantee",
    async () => {
      ...
      // Same peel-the-stack shape as migration-0055-roundtrip: one dbmate connection per step, O(migrations)
      // and growing. Count-scaled budget so it never drifts into the default 120s ceiling as migrations land.
    },
