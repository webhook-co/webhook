  ...
  return (
    <PageContainer>
      <div className="flex flex-col gap-1.5">
        <Link
          href={`/org/${session.slug}/endpoints`}
        ...
        <>
          ...
          {secrets.status === "error" ? (
            ...
            <ProviderSecretsManager
              ...
              add={addProviderSecretAction.bind(null, session.slug)}
              revoke={revokeProviderSecretAction.bind(null, session.slug)}
            />
          )}
          <EndpointDedupManager
            endpointId={result.endpoint.id}
            initial={result.endpoint.dedupConfig}
            update={updateEndpointDedupAction.bind(null, session.slug)}
          />
        </>
      )}
    </PageContainer>
