...
// A SOCKET-DEATH RECONNECT AFTER A LONG GAP MUST START FRESH — the path the hook's visibility bound cannot see.
//
// macOS suspend does not always fire visibilitychange, so `active` never flips and the hook never re-creates
// the session. The socket dies on TCP timeout and scheduleReconnect reuses the STICKY sessionId — and the
// DO's existing-binding branch resumes from its durable cursor, IGNORING whatever the client sends. So the
// client's only lever is to drop the sessionId, forcing a fresh DO that honours `since=now`. Without this, a
// weekend-asleep tail floods the whole weekend on wake: the exact history-replay this feature removes.
describe("createLiveEventsSession — a stale reconnect drops the sticky session and goes live", () => {
  function staleHarness() {
    ...
    return {
      ...
      start: () => h.start({ nowFn: () => clock, maxResumeGapMs: 60_000 }),
    };
  }

  it("a reconnect within the gap keeps the sticky session (seamless resume)", async () => {
    const h = staleHarness();
    const session = h.start();
    await flush();
    const first = FakeWebSocket.instances[0];
    first.ready("sess-live");
    first.message(summaryFrame(EVENT_ID, "cur-1"));
    h.advance(30_000); // under the 60s gap
    first.fireClose();
    h.timers[0]!();
    await flush();

    const second = FakeWebSocket.instances[1];
    expect(second.url).toContain("sessionId=sess-live");
    session.stop();
  });

  it("a reconnect after the gap drops the session and asks for since=now", async () => {
    const h = staleHarness();
    const session = h.start();
    await flush();
    const first = FakeWebSocket.instances[0];
    first.ready("sess-live");
    first.message(summaryFrame(EVENT_ID, "cur-1"));
    h.advance(60_001); // the lid was shut past the gap
    first.fireClose();
    h.timers[0]!();
    await flush();

    const second = FakeWebSocket.instances[1];
    expect(second.url).not.toContain("sessionId=");
    expect(second.url).toContain("since=now");
    expect(second.url).not.toContain("sinceCursor");
    session.stop();
  });
});
