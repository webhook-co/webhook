...
// The events-list filter bar: a provider dropdown + a received-at date range, driven entirely by the
// URL query (`?provider=&from=&to=`) so the filtered view is shareable, bookmarkable, and refresh-safe.
// Each change does a `router.replace` (no history spam, no scroll jump); the server page re-reads the
// query, re-runs the filtered load, and re-keys the list. No client-side filtering — the DB does it.

const FILTER_KEYS = ["provider", "from", "to"] as const;
