...
describe("createTriggerAction", () => {
  it("creates a trigger (orgId + actor from the session), strips orgId, revalidates", async () => {
    ...
    const res = await createTriggerAction({ endpointId: EP, name: "agent" });
    expect(res).toEqual({
      ok: true,
    ...
    expect(revalidatePath).toHaveBeenCalledWith("/triggers");
  });

  it("rejects a non-uuid endpoint id WITHOUT calling the mutation", async () => {
    const res = await createTriggerAction({ endpointId: "not-a-uuid" });
  ...
  it("rejects an over-long label", async () => {
    const res = await createTriggerAction({ endpointId: EP, name: "x".repeat(101) });
  ...
  it("maps an endpoint-not-found to a no-leak message", async () => {
    ...
    const res = await createTriggerAction({ endpointId: EP });
...
describe("revokeTriggerAction", () => {
  it("revokes and revalidates", async () => {
    ...
    const res = await revokeTriggerAction(TRIG);
  ...
  it("rejects a non-uuid id WITHOUT calling the mutation", async () => {
    const res = await revokeTriggerAction("nope");
