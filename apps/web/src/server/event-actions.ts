...
export async function loadMoreEventsAction(input: {
  ...
  // independently callable). Validate both fields — a bad value is also RLS-bounded + caught below, but
  // reject fast. `orderKey` is the UTC ISO-µs order key (see cursor.ts). We FAIL CLOSED on any other shape
  // (e.g. a legacy {receivedAt} cursor from a stale pre-deploy tab): a ms→µs boundary can't be losslessly
  // upgraded onto the DESC keyset without either skipping or re-showing same-ms rows, so — like the signed
  // v1 cursor — we reject it. "Load more" stops until the user reloads and gets fresh v2 cursors; a full
  // reload always renders the correct list, so nothing is ever silently dropped.
