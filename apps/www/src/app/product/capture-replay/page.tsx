    ...
    >
      <ProductFeature id="durable" heading="Durable before the acknowledgement">
        <p>
          ...
          pipeline acks first and persists later; we don&rsquo;t. So a crash on a malformed payload,
          or a bad deploy on your side, is a replay, not a lost event.
        </p>
      </ProductFeature>
      ...
      <ProductFeature id="replay" heading="Replay to localhost, one command">
        <p>
          ...
          provider &mdash; not one tool per provider. Replay a captured event to your laptop or back
          to production whenever you&rsquo;ve fixed the handler.
        </p>
      </ProductFeature>

      <ProductFeature id="accepts" heading="Accepts whatever a provider sends">
        <p>
          Providers don&rsquo;t agree on much. The ingest endpoint takes every HTTP method a webhook
          might arrive as, plus the <code>GET</code> verification handshake some providers require
          before they&rsquo;ll send anything &mdash; so you don&rsquo;t lose the first event to a
          setup check.
        </p>
      </ProductFeature>
    </ProductShell>
