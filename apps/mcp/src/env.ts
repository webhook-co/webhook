// Bindings for the mcp. OAuth issuer+resource Worker. The grant carries the org
// context in its props, so the OAuth/auth layer needs ONLY the provider's KV store — no DB. The
// DB/Hyperdrive bindings arrive with the MCP tool-dispatch binding (added separately), where
