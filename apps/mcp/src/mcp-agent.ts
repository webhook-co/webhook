...
const TOOL_DESCRIPTIONS: Record<string, string> = {
  ...
  "events.list":
    "List received events for an endpoint (paginated, newest-first). Optional filters (AND together): `filter.provider`; `filter.verificationState` (verified | failed | unattempted — failed = a signature was checked and rejected, unattempted = none was checked); a received-at range `filter.receivedAfter` (inclusive) / `filter.receivedBefore` (exclusive), each an RFC 3339 timestamp; and `filter.search`, a case-insensitive substring over the event's provider event id / external id / dedup key (and an exact match on the event id when given a uuid).",
