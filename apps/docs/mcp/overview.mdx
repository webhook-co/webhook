...
webhook.co speaks the Model Context Protocol. Point an MCP-capable agent at `mcp.webhook.co`, authenticate with a bearer token, and it can manage endpoints, read events, inspect deliveries, verify your audit chain — and, uniquely, be _woken_ by an inbound webhook.
