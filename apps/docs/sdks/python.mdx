...
The Python SDK returns validated pydantic v2 models with typed, snake_cased attributes. Python 3.10+, built
on `httpx`.
...
The client retries idempotent requests on transient failures with capped exponential backoff and jitter,
honouring `Retry-After`, and never blind-retries a non-idempotent write. Replays carry an idempotency key
