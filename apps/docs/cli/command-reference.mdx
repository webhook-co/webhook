...
`--provider` limits routing to one provider (default: any). `--event-type` selects event-type patterns — exact, `charge.*`, or `*` — repeatable or comma-separated (default `*`). `--require-verified` routes only signature-checked events.
