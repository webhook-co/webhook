...
<Note>
Deduplication is best-effort **load reduction**, not an exactly-once guarantee. Keep your own handler
idempotent — dedup reduces duplicates, it doesn't promise their absence.
</Note>
...
<Warning>
`off` means **every** request is a distinct, billable event — including retries and any non-webhook
traffic (uptime checks, link previews) that reaches the URL. Use it deliberately.
</Warning>
...
```sh API
# create with a dedup config
curl -X POST https://api.webhook.co/v1/endpoints \
  -H "authorization: Bearer $WBHK_API_KEY" \
  -H "content-type: application/json" \
  -d '{"name":"orders-prod","dedupConfig":{"mode":"content","windowSeconds":300}}'

# update an existing endpoint (null resets to the default)
