...
<Steps>
  ...
  <Step title="Deliver only what you verified">
    `--require-verified` forwards signature-checked events only; an `unattempted` event is never signed, and a `failed` one is blocked. Your receiver can trust that a signed delivery is genuine.
