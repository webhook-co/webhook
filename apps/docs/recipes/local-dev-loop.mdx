...
<Note>
  An edited replay won't re-verify against the provider's original signature — you're exercising handler logic, not verification. Replay without `--edit` to send the exact captured bytes.
</Note>
