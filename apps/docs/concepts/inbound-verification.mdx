...
- **`verified`** — a cryptographic signature over the payload checked out. This is the strong state, and the only one webhook.co will [re-sign](/concepts/delivery-retry-signing) on the way out.
...
That last distinction matters downstream: an `unattempted` event is delivered **unsigned** even to a destination configured for signing. webhook.co signs what it verified, and nothing else.
