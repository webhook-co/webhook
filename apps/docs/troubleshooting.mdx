...
<AccordionGroup>
  ...
  <Accordion title="The provider's setup handshake is failing">
    Handshakes are answered automatically, but some need a secret registered first — a Meta `verify_token`, a Braintree public key. Register the secret the [provider guide](/providers/directory) calls for, then retry the provider's verification.
  </Accordion>
