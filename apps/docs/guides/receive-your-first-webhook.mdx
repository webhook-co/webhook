...
Set the ingest URL as the webhook destination in your provider's dashboard — the [144 supported providers](/providers/directory) and anything else. The URL accepts every HTTP verb and answers provider verification handshakes automatically, so most providers validate it on the spot.
