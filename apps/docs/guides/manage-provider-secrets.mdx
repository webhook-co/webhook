...
<Warning>
  Revocation takes effect immediately — events signed only with the revoked secret start failing verification at once. During a rotation, add the new secret **before** you revoke the old one.
</Warning>
