...
<Note>
  webhook.co picks the right scheme by **which secrets you've registered**, not by trusting a header the sender set — so providers that share a signature header each verify correctly. It checks the registered providers against the captured bytes and the first that verifies names the event's provider.
</Note>
...
```sh
wbhk events list <endpoint-id> --verification-state verified --verification-state failed
```
...
<Warning>
  Always verify over the **raw request body**, byte-for-byte, before any JSON parsing or re-serialization — re-encoding changes the bytes and breaks the signature. And compare digests with a **constant-time** function (`crypto.timingSafeEqual`, `hmac.compare_digest`), never `==`. This is the single most common mistake in webhook verification.
</Warning>
